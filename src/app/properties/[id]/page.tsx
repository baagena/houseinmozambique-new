...
import { getPropertyById, getAgentById, properties, formatPrice } from '@/lib/dummyData';
...
export default async function PropertyDetailPage({ params }: Props) {
  ...
  const agent = getAgentById(property.hostId);
  const priceLabel = property.priceUnit === 'monthly' ? 'month' : property.priceUnit === 'nightly' ? 'night' : null;
  const similar = properties.filter((p) => p.id !== property.id && p.listingType === property.listingType).slice(0, 3);
              ...
              <div className="mb-16 pt-16 border-t border-[#c4c6cf]/20">
                <div className="flex items-start gap-6">
                  ...
                  <div>
                    ...
                    <p className="text-[#43474e] mb-4">{agent.title} · Hosting for {agent.yearsExperience} years</p>
                    ...
                    {agent.bio && (
                      <p className="text-[#43474e] italic mb-6 leading-relaxed max-w-lg">{agent.bio}</p>
                    )}
                    <Link
                      href="/agents"
            ...
            <div className="sticky top-28 bg-white p-8 rounded-xl shadow-[0_12px_32px_rgba(25,28,30,0.06)] ring-1 ring-[#c4c6cf]/10">
              <div className="flex justify-between items-end mb-8">
                <div>
                  ...
                  {priceLabel && <span className="text-[#43474e] font-medium"> / {priceLabel}</span>}
                </div>
                {property.rating && (
              ...
              {property.priceUnit === 'monthly' && (
                <div className="mt-8 pt-6 border-t border-[#c4c6cf]/20 space-y-4">
                  <div className="flex justify-between text-[#191c1e]">
                    <span>Monthly rent</span>
                    <span className="font-bold">{formatPrice(property.price, 'monthly')}</span>
                  </div>
                  <div className="flex justify-between text-[#191c1e]">
                    <span>Security deposit</span>
                    <span className="font-bold text-[#43474e]">{formatPrice(property.price, 'monthly')}</span>
                  </div>
                  <div className="flex justify-between text-[#191c1e] pt-4 border-t border-[#c4c6cf]/20">
                    <span className="font-extrabold">Total Move-in Cost</span>
                    <span className="font-extrabold text-[#002045]">{formatPrice(property.price * 2, 'sale')}</span>
                  </div>
                </div>
              )}
            </div>
